...
- **4-Stage Pipeline**:
  1. **Domain Discovery**: Analyzes Dart source code to document business logic.
...
```bash
python3 -m orchestrator_core.main --init
```
...
```bash
python3 -m orchestrator_core.main --module <your_module> --flow "<business_flow_name>"
```
...
```bash
python3 -m orchestrator_core.evaluator --stage stage_3 --file "e2e_test/modules/.../Happy Path.flow.yaml"
```
